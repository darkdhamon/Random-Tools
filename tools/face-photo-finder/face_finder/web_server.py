...
function renderFaceAssignment(face) {
  ...
  return `<div class="face-assignment face-visual-assignment"><div class="unknown-identity-picker"><input id="${inputId}" placeholder="Type or choose an identity" autocomplete="off" onfocus="activateFaceReticle(${face.id});openFaceIdentityPicker(${face.id},'${inputId}')" oninput="openFaceIdentityPicker(${face.id},'${inputId}')" onkeydown="if(event.key==='Enter'){event.preventDefault();assignFaceFromInput(${face.id},'${inputId}')}else if(event.key==='Escape')document.getElementById('${inputId}Choices').classList.remove('open')" aria-label="Assign ${htmlAttribute(faceDisplayName(face))}"><div id="${inputId}Choices" class="unknown-identity-choices"></div></div><button onclick="assignFaceFromInput(${face.id},'${inputId}')">Assign</button></div>`;
}
function openFaceIdentityPicker(faceId,inputId) {
  document.querySelectorAll('.unknown-identity-choices.open').forEach(item=>item.classList.remove('open'));
  const input=document.getElementById(inputId),choices=document.getElementById(`${inputId}Choices`),face=(current.faces||[]).find(item=>item.id===faceId),query=input.value.trim().toLocaleLowerCase(),allSuggestions=face?.assignment_suggestions||identities.map(identity=>({...identity,same_day:false,match_score:null,reference_face_id:null})),selected=allSuggestions.find(item=>item.id===+input.dataset.identityId);if(!selected||assignmentBaseLabel(selected).toLocaleLowerCase()!==query)delete input.dataset.identityId;
  const suggestions=allSuggestions.filter(item=>!query||item.name.toLocaleLowerCase().includes(query)||assignmentBaseLabel(item).toLocaleLowerCase().includes(query)||String(item.id)===query);
  choices.innerHTML=suggestions.map(item=>`<button type="button" class="unknown-identity-choice" onclick="selectFaceIdentity(${faceId},'${inputId}',${item.id})">${item.reference_face_id==null?'<span class="unknown-match-placeholder">?</span>':`<img loading="lazy" src="/api/identity-reference?id=${item.reference_face_id}" alt="">`}<span><strong>${esc(item.name)} (#${item.id})</strong><small>${item.same_day?'Same day · ':''}${item.match_score==null?'No biometric sample':`${(item.match_score*100).toFixed(1)}% visual match`}</small></span></button>`).join('')+(query&&!suggestions.some(item=>item.name.toLocaleLowerCase()===query)?`<button type="button" class="unknown-identity-choice create" onclick="keepNewFaceIdentityName('${inputId}')"><span class="unknown-match-placeholder">+</span><span><strong>Create “${esc(input.value.trim())}”</strong><small>New identity</small></span></button>`:'');choices.classList.add('open');observeDeferredImages(choices);
}
function selectFaceIdentity(faceId,inputId,identityId){const face=(current.faces||[]).find(item=>item.id===faceId),identity=(face?.assignment_suggestions||identities).find(item=>item.id===identityId),input=document.getElementById(inputId);input.value=assignmentBaseLabel(identity);input.dataset.identityId=identityId;document.getElementById(`${inputId}Choices`).classList.remove('open')}
function keepNewFaceIdentityName(inputId){document.getElementById(`${inputId}Choices`).classList.remove('open');document.getElementById(inputId).focus()}
async function assignFaceFromInput(faceId, inputId) {
  ...
  let identity = suggestions.find(item => item.id === +input.dataset.identityId) || suggestions.find(item => assignmentBaseLabel(item).toLocaleLowerCase() === value.toLocaleLowerCase());
...
function renderLocationOverview(){
  ...
  for(const layer of selectedMapLayers(locationOverviewMapStyle.value,locationOverviewRoads.checked,locationOverviewLabels.checked))for(let ty=firstY;ty<=lastY;ty++)for(let tx=firstX;tx<=lastX;tx++){if(ty<0||ty>=count)continue;const wrappedX=((tx%count)+count)%count;tiles+=`<img class="geofence-tile" alt="" src="${mapTileUrl(layer,zoom,wrappedX,ty)}" style="left:${tx*256-(centerX-width/2)}px;top:${ty*256-(centerY-height/2)}px">`}
...
function selectedMapLayers(style,roads=true,labels=true){const layers=style==='road'?['road']:['satellite'];if(style==='hybrid'&&roads)layers.push('roads');if(style==='hybrid'&&labels)layers.push('labels');return layers}
function renderPhotoMapTiles(){
  const lat=Number(latitude.value),lon=Number(longitude.value),valid=Number.isFinite(lat)&&Number.isFinite(lon)&&lat>=-90&&lat<=90&&lon>=-180&&lon<=180;if(!valid){photoMapTiles.innerHTML='';map.classList.remove('has-tiles');return}
  const zoom=13,width=map.clientWidth||400,height=map.clientHeight||150,center=overviewWorldPoint(lat,lon,zoom),count=2**zoom,firstX=Math.floor((center.x-width/2)/256),lastX=Math.floor((center.x+width/2)/256),firstY=Math.floor((center.y-height/2)/256),lastY=Math.floor((center.y+height/2)/256);let html='';for(const layer of selectedMapLayers(photoMapStyle.value,photoMapRoads.checked,photoMapLabels.checked))for(let ty=firstY;ty<=lastY;ty++)for(let tx=firstX;tx<=lastX;tx++){if(ty<0||ty>=count)continue;const wrappedX=((tx%count)+count)%count;html+=`<img class="geofence-tile" alt="" src="${mapTileUrl(layer,zoom,wrappedX,ty)}" style="left:${tx*256-(center.x-width/2)}px;top:${ty*256-(center.y-height/2)}px">`}photoMapTiles.innerHTML=html;map.classList.add('has-tiles');
}
const updateMapWithTileLayers=updateMap;updateMap=function(){updateMapWithTileLayers();renderPhotoMapTiles()};
function renderMapTiles(){
  ...
  const layers=selectedMapLayers(geofenceMapStyle.value,geofenceRoadOverlay.checked,geofenceLabelOverlay.checked);
...
PAGE = PAGE.replace(
    ...
    r'''<section id=albumView class=album-view><div class=album-management-toolbar><h1>Albums</h1><button onclick=createManagedAlbum()>New album...</button><button onclick=loadAlbumManagement()>Refresh</button></div><p class=muted>Album names save automatically. Open an album to browse its photos and locations.</p><div id=albumManagementStatus class=identity-status></div><div id=albumManagementGrid class=album-management-grid></div></section><div id=albumDetailDialog class="danger-dialog album-detail-dialog" role=dialog aria-modal=true aria-labelledby=albumDetailTitle><div class=album-detail><div class=album-detail-header><h2 id=albumDetailTitle>Album</h2><div class=compact-map-layers><select id=albumDetailMapStyle onchange=renderAlbumDetailMap()><option value=hybrid>Satellite + roads</option><option value=satellite>Satellite</option><option value=road>Road map</option></select><label><input id=albumDetailRoads type=checkbox checked onchange=renderAlbumDetailMap()> Roads</label><label><input id=albumDetailLabels type=checkbox checked onchange=renderAlbumDetailMap()> Labels</label></div><button onclick=closeAlbumViewer()>Close</button></div><aside class=album-detail-side><div id=albumDetailMap class=album-detail-map><div id=albumDetailTiles class=geofence-tiles aria-hidden=true></div><svg id=albumDetailMarkers preserveAspectRatio=none aria-label="Photo locations in this album"></svg></div><section class=album-detail-recommendations><h3>Recommended photos</h3><div id=albumDetailRecommendations></div></section></aside><div class=album-detail-stage><img id=albumDetailImage alt="Selected album photo"><div class=album-detail-caption><strong id=albumDetailName></strong><span id=albumDetailDate class=muted></span><button onclick=editAlbumViewerPhoto()>Edit photo</button><button onclick=removeAlbumViewerPhoto()>Remove from album</button></div></div><div id=albumDetailStrip class=album-detail-strip aria-label="Album photos in chronological order"></div></div></div><section id=locationView class=location-view>''',
    1,
).replace(
    '</style>',
    r'''.album-view{display:none;padding:22px;max-width:1500px;margin:auto}.album-management-toolbar{display:flex;align-items:center;gap:10px;flex-wrap:wrap}.album-management-toolbar h1{margin-right:auto}.album-management-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(300px,1fr));gap:14px}.album-management-card{background:#1c2528;border:1px solid #3d5960;border-radius:9px;padding:12px}.album-management-card input{font-size:1.15rem;font-weight:700;width:100%;box-sizing:border-box}.album-management-previews{display:grid;grid-template-columns:repeat(4,1fr);gap:4px;margin:10px 0}.album-management-previews img{width:100%;aspect-ratio:1;object-fit:cover;border-radius:4px}.album-recommendations{margin:10px 0;padding:8px;background:#172f34;border:1px solid #467780;border-radius:7px}.album-recommendation-grid{display:grid;grid-template-columns:repeat(5,1fr);gap:5px;margin:8px 0;max-height:250px;overflow:auto}.album-recommendation-grid label{position:relative}.album-recommendation-grid input{position:absolute;width:auto;left:4px;top:4px;z-index:1}.album-recommendation-grid img{width:100%;aspect-ratio:1;object-fit:cover;border-radius:4px}.album-management-people{margin:9px 0}.album-management-people-list{display:flex;gap:5px;flex-wrap:wrap;margin-top:5px}.album-person-chip{padding:4px 7px;font-size:.88rem;border-color:#52747c;background:#26383c}.album-management-actions{display:flex;align-items:center;gap:8px;flex-wrap:wrap}.album-management-save{min-height:1.3em;color:#8fdce7}.album-detail-dialog.open{display:flex}.album-detail{width:min(1500px,96vw);height:min(920px,94vh);background:#171e20;border:1px solid #52747c;border-radius:10px;padding:12px;display:grid;grid-template-columns:minmax(0,1fr) minmax(300px,34%);grid-template-rows:auto minmax(180px,32%) minmax(0,1fr) 150px;gap:10px}.album-detail-header{grid-column:1/-1;display:flex;align-items:center;justify-content:space-between}.album-detail-header h2{margin:0}.album-detail-map{grid-column:2;grid-row:2/4;position:relative;overflow:hidden;border:1px solid #4d6b72;border-radius:8px;background:#173039}.album-detail-map>svg{position:absolute;inset:0;width:100%;height:100%}.album-detail-marker{cursor:pointer;fill:#0b7085;stroke:#d1fbff;stroke-width:3}.album-detail-marker.selected{fill:#e0a62e;stroke:white}.album-detail-stage{grid-column:1;grid-row:2/4;min-width:0;min-height:0;background:#080808;border-radius:8px;display:flex;flex-direction:column;align-items:center;justify-content:center;overflow:hidden}.album-detail-stage img{min-height:0;max-width:100%;max-height:calc(100% - 52px);object-fit:contain}.album-detail-caption{width:100%;box-sizing:border-box;padding:8px;display:flex;align-items:center;gap:9px}.album-detail-caption .muted{margin-right:auto}.album-detail-strip{grid-column:1/-1;display:flex;gap:7px;overflow-x:auto;padding:5px;background:#101516;border-radius:7px}.album-detail-thumb{flex:0 0 130px;padding:3px;text-align:left}.album-detail-thumb.selected{border-color:#62e0ef;background:#195661}.album-detail-thumb img{display:block;width:100%;height:88px;object-fit:cover;border-radius:4px}.album-detail-thumb span{display:block;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;font-size:11px;margin-top:3px}@media(max-width:800px){.album-view{padding:12px}.album-management-grid{grid-template-columns:1fr}.album-detail{grid-template-columns:1fr;grid-template-rows:auto 180px minmax(260px,1fr) 135px}.album-detail-map{grid-column:1;grid-row:2}.album-detail-stage{grid-column:1;grid-row:3}.album-detail-strip{grid-row:4}}</style>''',
    1,
).replace(
    '</style>',
    r'''.album-detail-side{grid-column:2;grid-row:2/4;display:grid;grid-template-rows:minmax(180px,1fr) minmax(150px,42%);gap:10px;min-height:0}.album-detail-side .album-detail-map{grid-column:auto;grid-row:auto;min-height:0}.album-detail-recommendations{min-height:0;overflow:auto;border:1px solid #4d6b72;border-radius:8px;background:#14292e;padding:9px}.album-detail-recommendations h3{margin:0 0 8px}.album-detail-recommendation-grid{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:6px}.album-detail-recommendation{position:relative;padding:3px;text-align:left}.album-detail-recommendation input{position:absolute;z-index:1;left:7px;top:7px}.album-detail-recommendation img{display:block;width:100%;aspect-ratio:1;object-fit:cover;border-radius:4px}.album-detail-recommendation span{display:block;font-size:10px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.album-detail-recommendation-actions{display:flex;align-items:center;gap:7px;margin-top:8px}.album-detail-caption button:last-child{background:#68343a;border-color:#b65b64}@media(max-width:800px){.album-detail-side{grid-column:1;grid-row:2;grid-template-columns:1fr 1fr;grid-template-rows:180px}.album-detail-stage{grid-row:3}.album-detail-recommendation-grid{grid-template-columns:repeat(2,minmax(0,1fr))}}</style>''',
    1,
).replace(
    '</body>',
    r'''<script>
const albumNameTimers=new Map();
let albumDetailPhotos=[],albumDetailIndex=0,albumDetailAlbum=null;
...
async function albumPhotos(albumId){let photos=[],offset=0;while(true){const batch=await api(`/api/photos?album_id=${albumId}&nsfw=all&limit=250&offset=${offset}`);photos.push(...batch);if(batch.length<250)break;offset+=batch.length}return photos.sort((left,right)=>(left.capture_date||`${left.year||9999}-12-31`).localeCompare(right.capture_date||`${right.year||9999}-12-31`)||left.id-right.id)}
async function openAlbumViewer(albumId){albumDetailAlbum=albums.find(album=>album.id===albumId);albumDetailTitle.textContent=albumDetailAlbum?.name||'Album';albumDetailStrip.innerHTML='<span class="muted">Loading photos…</span>';albumDetailDialog.classList.add('open');albumDetailPhotos=await albumPhotos(albumId);albumDetailIndex=0;renderAlbumViewer()}
function closeAlbumViewer(){albumDetailDialog.classList.remove('open');albumDetailImage.removeAttribute('src')}
document.addEventListener('keydown',event=>{if(event.key==='Escape'&&albumDetailDialog.classList.contains('open'))closeAlbumViewer()});
function renderAlbumViewer(){
  renderAlbumDetailRecommendations();
  if(!albumDetailPhotos.length){albumDetailImage.removeAttribute('src');albumDetailName.textContent='No photos in this album';albumDetailDate.textContent='';albumDetailStrip.innerHTML='';albumDetailTiles.innerHTML='';albumDetailMarkers.innerHTML='';return}
  const selected=albumDetailPhotos[albumDetailIndex];albumDetailImage.src=`/media?id=${selected.id}${privacySuffix()}`;albumDetailName.textContent=selected.title||selected.name;albumDetailDate.textContent=selected.capture_date||selected.year||'Unknown date';
  albumDetailStrip.innerHTML=albumDetailPhotos.map((photo,index)=>`<button class="album-detail-thumb ${index===albumDetailIndex?'selected':''}" onclick="selectAlbumPhoto(${index})"><img loading="lazy" src="/media?id=${photo.id}&thumb=1${privacySuffix()}" alt=""><span>${esc(photo.capture_date||String(photo.year||'Unknown date'))} · ${esc(photo.title||photo.name)}</span></button>`).join('');renderAlbumDetailMap();
}
function selectAlbumPhoto(index){albumDetailIndex=index;renderAlbumViewer();const selected=albumDetailStrip.querySelector('.selected');if(selected)selected.scrollIntoView({block:'nearest',inline:'center'})}
function editAlbumViewerPhoto(){if(!albumDetailPhotos.length)return;const id=albumDetailPhotos[albumDetailIndex].id;closeAlbumViewer();openPhoto(id)}
function renderAlbumDetailRecommendations(){
  const recommended=albumDetailAlbum?.recommended_photos||[];
  albumDetailRecommendations.innerHTML=recommended.length?`<div class="album-detail-recommendation-grid">${recommended.map(photo=>`<label class="album-detail-recommendation" title="${esc(photo.capture_date+' · '+photo.name)}"><input type="checkbox" name="albumDetailRecommendation" value="${photo.id}" checked><img loading="lazy" src="/media?id=${photo.id}&thumb=1${privacySuffix()}" alt="${esc(photo.name)}"><span>${esc(photo.capture_date)} · ${esc(photo.name)}</span></label>`).join('')}</div><div class="album-detail-recommendation-actions"><button onclick="addAlbumDetailRecommendations()">Add selected</button><span class="muted">${recommended.length} nearby</span></div>`:'<span class="muted">No nearby ungrouped photos.</span>';
}
async function refreshOpenAlbum(preferredPhotoId=null){
  await loadAlbums();albumDetailAlbum=albums.find(album=>album.id===albumDetailAlbum.id);albumDetailPhotos=await albumPhotos(albumDetailAlbum.id);const preferredIndex=albumDetailPhotos.findIndex(photo=>photo.id===preferredPhotoId);albumDetailIndex=preferredIndex>=0?preferredIndex:Math.min(albumDetailIndex,Math.max(0,albumDetailPhotos.length-1));renderAlbumViewer();
}
async function addAlbumDetailRecommendations(){
  const ids=[...albumDetailRecommendations.querySelectorAll('input[name="albumDetailRecommendation"]:checked')].map(input=>+input.value);if(!ids.length)return;await post('/api/add-album-recommendations',{album_id:albumDetailAlbum.id,image_ids:ids});await refreshOpenAlbum(ids[0]);
}
async function removeAlbumViewerPhoto(){
  if(!albumDetailPhotos.length)return;const photo=albumDetailPhotos[albumDetailIndex];await post('/api/remove-photo-from-album',{album_id:albumDetailAlbum.id,image_id:photo.id});await refreshOpenAlbum();
}
function renderAlbumDetailMap(){
  const located=albumDetailPhotos.map((photo,index)=>({photo,index})).filter(item=>item.photo.latitude!=null&&item.photo.longitude!=null),width=albumDetailMap.clientWidth||420,height=albumDetailMap.clientHeight||500;if(!located.length){albumDetailTiles.innerHTML='';albumDetailMarkers.innerHTML='<text x="20" y="35" fill="#ddd">No located photos in this album</text>';return}
  const normalized=located.map(item=>overviewWorldPoint(item.photo.latitude,item.photo.longitude,0)),minX=Math.min(...normalized.map(point=>point.x)),maxX=Math.max(...normalized.map(point=>point.x)),minY=Math.min(...normalized.map(point=>point.y)),maxY=Math.max(...normalized.map(point=>point.y));let zoom=1;for(let candidate=18;candidate>=1;candidate--){const scale=2**candidate;if((maxX-minX)*scale<=width-70&&(maxY-minY)*scale<=height-70){zoom=candidate;break}}if(located.length===1)zoom=13;const centerX=(minX+maxX)/2*2**zoom,centerY=(minY+maxY)/2*2**zoom,count=2**zoom,firstX=Math.floor((centerX-width/2)/256),lastX=Math.floor((centerX+width/2)/256),firstY=Math.floor((centerY-height/2)/256),lastY=Math.floor((centerY+height/2)/256);let tiles='';for(const layer of selectedMapLayers(albumDetailMapStyle.value,albumDetailRoads.checked,albumDetailLabels.checked))for(let ty=firstY;ty<=lastY;ty++)for(let tx=firstX;tx<=lastX;tx++){if(ty<0||ty>=count)continue;const wrappedX=((tx%count)+count)%count;tiles+=`<img class="geofence-tile" alt="" src="${mapTileUrl(layer,zoom,wrappedX,ty)}" style="left:${tx*256-(centerX-width/2)}px;top:${ty*256-(centerY-height/2)}px">`}albumDetailTiles.innerHTML=tiles;
  const groups=new Map();for(const item of located){const projected=overviewWorldPoint(item.photo.latitude,item.photo.longitude,zoom),x=projected.x-(centerX-width/2),y=projected.y-(centerY-height/2),key=`${Math.floor(x/38)}:${Math.floor(y/38)}`,group=groups.get(key)||{x:0,y:0,indices:[]};group.x+=x;group.y+=y;group.indices.push(item.index);groups.set(key,group)}albumDetailMarkers.setAttribute('viewBox',`0 0 ${width} ${height}`);albumDetailMarkers.innerHTML=[...groups.values()].map(group=>{const x=group.x/group.indices.length,y=group.y/group.indices.length,selected=group.indices.includes(albumDetailIndex),index=group.indices[0],count=group.indices.length;return `<g role="button" tabindex="0" onclick="selectAlbumPhoto(${index})"><circle class="album-detail-marker ${selected?'selected':''}" cx="${x}" cy="${y}" r="${count>1?16:11}"></circle>${count>1?`<text class="location-map-count" x="${x}" y="${y}">${count}</text>`:''}</g>`}).join('');
}
function renderAlbumManagement(){
  albumManagementStatus.textContent=`${albums.length} album${albums.length===1?'':'s'}`;
  albumManagementGrid.innerHTML=albums.length?albums.map(album=>`<article class="album-management-card"><input aria-label="Album name" value="${esc(album.name)}" oninput="queueAlbumNameSave(${album.id},this)"><div id="albumSave-${album.id}" class="album-management-save"></div><div class="muted">${album.photo_count} photo${album.photo_count===1?'':'s'} | ${esc(albumDateRange(album))}</div><div class="album-management-previews">${album.preview_photo_ids.map(id=>`<img loading="lazy" src="/media?id=${id}&thumb=1${privacySuffix()}" alt="">`).join('')}</div>${album.recommended_photos.length?`<details class="album-recommendations"><summary>${album.recommended_photos.length} nearby ungrouped photo${album.recommended_photos.length===1?'':'s'} recommended</summary><div class="album-recommendation-grid">${album.recommended_photos.map(photo=>`<label title="${esc(photo.capture_date+' · '+photo.name)}"><input type="checkbox" name="albumRecommendation-${album.id}" value="${photo.id}" checked><img loading="lazy" src="/media?id=${photo.id}&thumb=1${privacySuffix()}" alt="${esc(photo.name)}"></label>`).join('')}</div><button onclick="addAlbumRecommendations(${album.id})">Add selected photos</button></details>`:''}<div class="album-management-people"><strong>People (${album.people.length})</strong><div class="album-management-people-list">${album.people.length?album.people.map(identity=>`<button class="album-person-chip" onclick="viewAlbumPersonTimeline(${album.id},${identity.id})">${esc(identity.name)} (${identity.photo_count})</button>`).join(''):'<span class="muted">No identified people</span>'}</div></div><div class="album-management-actions"><button onclick="openAlbumViewer(${album.id})">View photos</button><button onclick="requestAlbumMissingLocation(${album.id})">Set missing locations</button></div></article>`).join(''):'<p>No albums yet. Create one to start organizing photos.</p>';
}
async function addAlbumRecommendations(albumId){const ids=[...document.querySelectorAll(`input[name="albumRecommendation-${albumId}"]:checked`)].map(input=>Number(input.value));if(!ids.length)return;try{const result=await post('/api/add-album-recommendations',{album_id:albumId,image_ids:ids});await loadAlbumManagement();albumManagementStatus.textContent=`Added ${result.added} recommended photo${result.added===1?'':'s'}.`}catch(error){albumManagementStatus.textContent='Could not add recommendations: '+error.message}}
...
function toggleNsfwVisibility(){
  ...
  else if(albumView.style.display==='block'){renderAlbumManagement();if(albumDetailDialog.classList.contains('open'))renderAlbumViewer()}
...
class GalleryHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self) -> None:
        ...
        try:
            ...
            elif self.path == "/api/add-album-recommendations":
                catalog = self._catalog()
                try:
                    added = catalog.add_album_recommendations(
                        int(body["album_id"]), [int(value) for value in body.get("image_ids", [])]
                    )
                finally: catalog.close()
                self._json({"ok": True, "added": added})
            elif self.path == "/api/remove-photo-from-album":
                catalog = self._catalog()
                try:
                    removed = catalog.remove_photo_from_album(
                        int(body["album_id"]), int(body["image_id"])
                    )
                finally: catalog.close()
