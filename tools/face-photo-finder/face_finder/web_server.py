    ...
    def do_POST(self) -> None:
        ...
        try:
            ...
            if self.path == "/api/photo":
                catalog = self._catalog()
                try:
                    catalog.update_gallery_metadata(int(body["id"]), str(body.get("title", "")), str(body.get("description", "")), str(body.get("tags", "")), int(body.get("rating", 0)), body.get("capture_year"), body.get("nsfw_override"))
                finally: catalog.close()
                self._json({"ok": True})
