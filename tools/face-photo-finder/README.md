...
- includes a dedicated Albums tab for viewing album previews, counts and date ranges, autosaving album-name edits, creating albums, and opening an album in the Timeline for photo membership editing;
...
- includes a responsive Locations map that starts from the device location (with browser permission) and falls back to Madison Lake, Minnesota; supports mouse-drag or WASD panning, Shift-click recentering, wheel or Q/E zooming, editable user-created radius and hand-drawn polygon geofences, and read-only imported legal boundaries; offers selectable hybrid satellite-and-roads, satellite, and road layers; and supports hierarchical parent locations, grouped previews, and location-filtered browsing;
- separates location browsing from geofence administration: Locations automatically fits an overview map to every geotagged photo and combines nearby photo pins into numbered clusters, while Settings contains the dedicated boundary creation and editing map;
- offers a quick selector on busy-day album suggestions for adding those ungrouped photos to an existing album that already contains photos taken within seven days;
- caches legal-boundary membership when an image is first processed and refreshes it only when that image's coordinates change; custom-geofence membership additionally refreshes when a custom boundary is created or edited, avoiding repeated geometry matching during gallery loads;
- keeps Settings focused on custom geofences, while the Locations tab shows only legal areas containing cataloged photos in a collapsible nation → state/region → county → city hierarchy beneath the photo map;
- opens map pins in a right-side photo drawer: individual pins show the photo directly, while clustered pins show selectable thumbnails for every photo in that map area;
- lets the photo viewer explicitly remove GPS metadata by storing null effective coordinates (never zero), suppressing embedded coordinates until new latitude and longitude values are entered;
- presents every custom and legal geofence membership in the photo viewer as a read-only GPS-derived result rather than a manually selectable tag;
- supports a spatially indexed North American administrative-boundary catalog. `python import-boundaries.py` imports 2025 U.S. Census TIGER/Line country/state/county/place polygons, 2025 Statistics Canada province/census-division/census-subdivision polygons, and 2025 INEGI state/municipal polygons. All imported areas remain searchable in the catalog, while the Locations viewer shows only boundaries containing cataloged photos;
