...
- includes a Locations tab that starts from the device location (with browser permission) and falls back to Madison Lake, Minnesota, with radius, hand-drawn polygon, and imported legal-boundary GeoJSON geofences; selectable hybrid satellite-and-roads, satellite, and road map layers; hierarchical parent locations; grouped photo previews; location-filtered browsing; and optional manual multi-location assignments for photos without suitable GPS data;
