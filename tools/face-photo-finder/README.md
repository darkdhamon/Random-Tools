...
To build or update the identity catalog, clear the reference photos, leave **Known person** blank, and start a scan. Every photo containing a face is included, and every detected face in a multi-person photo is checked independently. For an unfamiliar face, choose an existing identity, enter a new name, open the full photo for context, mark a false detection as **Not a face**, skip that face, or skip the remaining prompts for that scan. A normally skipped face remains unidentified and is offered again on the next all-faces scan. Names learned early in a scan are used to recognize later faces immediately.
