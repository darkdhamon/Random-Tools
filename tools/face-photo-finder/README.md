...
- includes a responsive Locations map that starts from the device location (with browser permission) and falls back to Madison Lake, Minnesota; supports mouse-drag or WASD panning, Shift-click recentering, wheel or Q/E zooming, radius, hand-drawn polygon, and imported legal-boundary GeoJSON geofences; offers selectable hybrid satellite-and-roads, satellite, and road layers; and supports hierarchical parent locations, grouped previews, location-filtered browsing, and optional manual multi-location assignments;
